...
class MetaLinearHeadBinary(nn.Module):
    ...
    def forward(
        self, Xs: torch.Tensor, ys: torch.Tensor, mask=None
    ...
    def predict_query(
        self, Xq: torch.Tensor, w: torch.Tensor, b: torch.Tensor
    ) -> torch.Tensor:
        if Xq.dim() == 2:
...
class MetaLinearHeadMC_Attn(nn.Module):
    ...
    def _masked_weighted_mean(
        self, Hs: torch.Tensor, ys: torch.Tensor, mask_s: torch.Tensor | None
    ):
        """
    ...
    def fit_support(
        ...
        assert support_feats.dim() == 2 and support_labels.dim() == 2
        S, E = support_feats.shape
        S2, C = support_labels.shape
        assert S == S2 and E == self.embed_dim

        Hs = self.phi(support_feats)  # (S,H)

        # initial class queries (prototype-like), then refine via cross-attention
        Q0 = self._masked_weighted_mean(Hs, support_labels, support_mask)  # (C,H)
        Rc = self.attn_pool(queries=Q0, support=Hs, mask_s=support_mask)  # (C,H)
