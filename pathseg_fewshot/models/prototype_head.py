...
class PrototypeHeadOld(nn.Module):
    """
    Prototype head on token features. w/o PANet style to handle bg.
...
class PrototypeHead(nn.Module):
    ...
    def fit_support(
        ...
        prototypes = self._compute_prototypes(feats, labels)  # [C,H]
...
class PrototypeHeadV2(nn.Module):
    ...
    def fit_support(
        ...
        if self.center:
            if support_mask is not None:
                m = support_mask.to(dtype=support_feats.dtype).unsqueeze(-1)
                denom = m.sum().clamp_min(self.eps)
        ...
        support_feats, labels = self._apply_mask(
            support_feats, support_labels, support_mask
        )
